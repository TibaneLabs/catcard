...
/// [`PAW`] drawn at three times the size rather than scaled to it: the Q1's print. 27x30.
///
/// The same print -- pad behind, four toes in an arc ahead, each where the small one has it
/// -- from ellipses at full size, so the edges are round instead of 3x3 blocks. Exactly
/// three times [`PAW`]'s footprint, so a trail, a card row and the caret sit exactly
/// where they did. Generated by `tools/artgen/paw.py`.
#[rustfmt::skip]
pub const PAW_3X: Bitmap = Bitmap {
    ...
    // ...............####........
    pixels: &[
        0x00, 0x01, 0xE0, 0x00, 0x00, 0x03, 0xF0, 0x00, 0x00, 0x03, 0xF0, 0x00, 0x00, 0x07, 0xF8, 0x00,
        0x00, 0x03, 0xF0, 0x00, 0x00, 0x03, 0xF0, 0x00, 0x00, 0x01, 0xE2, 0x00, 0x03, 0xE0, 0xCF, 0x80,
        0x1F, 0xFC, 0x1F, 0xC0, 0x3F, 0xFE, 0x1F, 0xC0, 0x7F, 0xFF, 0x1F, 0xC0, 0x7F, 0xFF, 0x1F, 0xC0,
        0xFF, 0xFF, 0x8F, 0x80, 0xFF, 0xFF, 0x02, 0x00, 0xFF, 0xFE, 0x00, 0x00, 0xFF, 0xFE, 0x00, 0x00,
        0xFF, 0xFF, 0x02, 0x00, 0xFF, 0xFF, 0x8F, 0x80, 0x7F, 0xFF, 0x1F, 0xC0, 0x7F, 0xFF, 0x1F, 0xC0,
        0x3F, 0xFE, 0x1F, 0xC0, 0x1F, 0xFC, 0x1F, 0xC0, 0x03, 0xE0, 0xCF, 0x80, 0x00, 0x01, 0xE2, 0x00,
        0x00, 0x03, 0xF0, 0x00, 0x00, 0x03, 0xF0, 0x00, 0x00, 0x07, 0xF8, 0x00, 0x00, 0x03, 0xF0, 0x00,
        0x00, 0x03, 0xF0, 0x00, 0x00, 0x01, 0xE0, 0x00,
    ],
};
...
mod tests {
    ...
    /// The large print is the small one's shape at three times: every cell the small one
    /// fills is mostly filled in the large, and the large puts nothing where the small one
    /// has no print at all. Mirror-symmetric top to bottom, as the small one is.
    #[test]
    fn the_large_print_is_the_small_one_drawn_finer() {
        for cy in 0..PAW.height as usize {
            for cx in 0..PAW.width as usize {
                let lit = (0..3)
                    .flat_map(|dy| (0..3).map(move |dx| (cx * 3 + dx, cy * 3 + dy)))
                    .filter(|&(x, y)| PAW_3X.pixel(x, y))
                    .count();
                if PAW.pixel(cx, cy) {
                    assert!(lit >= 3, "cell ({cx},{cy}) is thin: {lit} of 9");
                }
            }
        }
        let h = PAW_3X.height as usize;
        for y in 0..h {
            for x in 0..PAW_3X.width as usize {
                assert_eq!(PAW_3X.pixel(x, y), PAW_3X.pixel(x, h - 1 - y), "({x},{y})");
            }
        }
    }
