...
/// Whether the stack fence is armed: the MPU has a live no-access region and the callgate
/// wrapper must therefore drop and restore it across every gate call.
///
/// **False on every boot, and until [`arm_guard`] programs the fence.** This is the one
/// thing the callgate reads (`catcard_callgate::entry::with_interrupts_masked`) to decide
/// whether to touch an MPU register at all: while it is false the gate path is byte for
/// byte what it was before the fence existed -- mask, call, restore -- and reads no MPU
/// register. It is set true only once the fence is live, so a gate call can never let the
/// bootloader write the fenced bytes from inside the firewall while the callgate believes
/// there is nothing to protect.
static FENCE_ARMED: AtomicBool = AtomicBool::new(false);

/// Whether the stack fence is armed (see [`FENCE_ARMED`]). Read by the callgate wrapper.
#[inline]
pub fn fence_armed() -> bool {
    FENCE_ARMED.load(Ordering::Relaxed)
}
...
pub unsafe fn arm_guard(base: u32) -> Result<(), Error> {
    ...
    // The fence is live now, so the callgate must protect it from here on. Set after the
    // MPU is actually enabled -- and, in the arm sequence, `enable_memfault` runs next,
    // both inside the caller's interrupt-free section, so no gate call falls between the
    // enable and this store.
    FENCE_ARMED.store(true, Ordering::Relaxed);
    Ok(())
}
...
pub unsafe fn disarm() {
    ...
    // Only after the MPU is off: while the fence is still live the callgate must keep
    // protecting it, so the flag drops last. A gate call in the gap sees the flag set,
    // reads `MPU_CTRL` as already zero, and does nothing -- which is correct.
...
mod tests {
    ...
    #[test]
    fn the_fence_is_disarmed_before_anything_arms_it() {
        // The callgate reads this to decide whether to touch an MPU register; it must be
        // false on every boot, before `arm_guard` runs. `arm_guard`/`disarm` themselves
        // write MPU registers and so cannot run on the host -- only the default is
        // checkable here, which is the boot invariant that matters.
        assert!(!fence_armed());
    }
