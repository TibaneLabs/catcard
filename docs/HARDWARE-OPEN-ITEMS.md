...
## The Q1's lamp key has nowhere to go

`kn50` is LAMP. The reference is explicit that it is **torch-only, handled in the driver
and "never delivered as a char"** (`input.md` §"Key decode"), so it is correctly absent
from the decode tables -- but **no lamp or torch pin is documented for this board**. The
only backlight named anywhere is `BL_ENABLE=PE3`, which is the LCD's and already in the
board table.

So the key is scanned, debounced, and deliberately produces nothing. It is the one key on
the Q1 keyboard that still does nothing after the keyboard learned to type, and it cannot
be fixed by guessing: driving an unknown GPIO to find a lamp is exactly the kind of guess
`CLAUDE.md` forbids.

**How to resolve.** Find the torch LED's pin in `COLDCARD_Q1/pins.csv` or the schematic,
add it to `BoardSpec`, and have the driver toggle it on `kn50` -- no `Key` is involved,
since the reference says it never reaches the app as a character.

---

## The Q1's SYMBOL tables are ambiguous in the reference

SHIFT and CAPS are implemented; **SYMBOL is scanned and tracked but types nothing**. The
reference prints its rows as

```
SYMBOL: ... !@#$%^&*()   -_` []{}    + =:;~|\ "  F1..F6 <>?
```

and those letter rows do not line up unambiguously with ten positions each -- the spacing
is the only separator, and it does not divide evenly. Typing the wrong punctuation into a
passphrase is worse than typing none, so the tables stay empty until the mapping is
confirmed position by position.

**How to resolve.** Confirm `shared/charcodes.py`'s four 60-character strings against the
physical legends, then fill `SYMBOL_CHARS` in `catcard-ui/src/qwerty.rs` the way
`BASE_CHARS` and `SHIFT_CHARS` are filled.
