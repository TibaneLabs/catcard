...
//! tear pulse, by itself -- the Q1's equivalent of the OLED's hardware scroll. Nothing else
//! of it is used: no cursor, no firmware update.
//!
...
pub fn activity_bar() -> bool {
    ...
    match i2c.write(APP_ADDR, &[OP_ACTIVITY_BAR]) {
        Ok(()) => true,
        Err(e) => {
            crate::catlog!("gpu: activity bar refused: {:?}", e);
            false
        }
    }
}
