...
/// program, and every bench unit is RDP=2 with no recovery. So it is turned on from
/// *Debug -> Stack guard*, proven by its probe there, and stays a Debug action until it
/// has been proven on every board (docs/KERNEL.md §5).
...
/// wrong. It runs in PendSV and must not return; the firmware passes its
/// wipe-and-reset.
///
/// # Safety
